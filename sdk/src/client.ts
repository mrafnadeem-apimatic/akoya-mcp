...
export class Client implements ClientInterface {
  ...
  constructor(config?: Partial<Configuration>) {
    ...
    this._userAgent = updateUserAgent(
      'TypeScript SDK, Version: 2.4.0, on OS {os-info}'
    );
...
function getBaseUri(server: Server = 'default', config: Configuration): string {
  if (config.environment === Environment.Sandbox) {
    if (server === 'default') {
      return 'https://sandbox-products.ddp.akoya.com';
    }
    if (server === 'auth server') {
      return 'https://sandbox-idp.ddp.akoya.com';
    }
  }
  if (config.environment === Environment.Production) {
    if (server === 'default') {
      return 'https://products.ddp.akoya.com';
    }
    if (server === 'auth server') {
      return 'https://sandbox-idp.ddp.akoya.com';
    }
